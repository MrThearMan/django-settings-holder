...
.PHONY: help serve-docs build-docs submit-docs tests test tox hook pre-commit black isort pylint flake8 mypy Makefile
...
pylint:
	@poetry run pylint settings_holder/

flake8:
	@poetry run flake8 --max-line-length=120 --extend-ignore=E203,E501 settings_holder/

mypy:
	@poetry run mypy settings_holder/
